# GRAAL-ATACK OS Builder

Ce dossier contient les éléments nécessaires pour transformer une Debian/Kali minimale en **GRAAL-ATACK OS** : un environnement graphique léger Openbox/LightDM qui lance automatiquement GRAAL-ATACK en mode kiosk réversible.
...
2. copie l'application dans `/opt/graal-atack` ;
...
8. installe `/etc/motd` ;
9. sauvegarde `/etc/os-release` puis installe le branding GRAAL-ATACK.
...
Copiez ensuite le dépôt GRAAL-ATACK dans l'image chroot :
...
L'ISO générée démarre sur LightDM/Openbox et lance GRAAL-ATACK via l'autostart configuré par `postinstall.sh`.
