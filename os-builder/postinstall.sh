...
if [[ -d "${REPO_DIR}/assets" ]]; then
  cp -a "${REPO_DIR}/assets/." "${APP_DIR}/assets/"
else
  echo "Dossier assets optionnel absent: placeholders Unicode utilisés."
fi
...
if [[ -d /etc/lightdm ]]; then
  ...
  cat > /etc/lightdm/lightdm.conf.d/50-graal-kiosk.conf <<EOF
[Seat:*]
...
if [[ -f /etc/os-release && ! -f /etc/os-release.graal-backup ]]; then
  cp /etc/os-release /etc/os-release.graal-backup
fi
