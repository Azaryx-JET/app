...
## Polices personnalisées optionnelles

L'application scanne automatiquement `assets/fonts/` au démarrage. Les familles recommandées sont privilégiées si elles sont installées dans l'environnement Tk ou si les fichiers sont fournis avec des noms explicites. Aucun fichier de police n'est obligatoire : le fallback reste `Segoe UI` / `Arial`.
...
- **Titres majeurs** : Cinzel, Cormorant Garamond, Marcellus SC, Georgia.
- **Titres secondaires** : Cinzel SemiBold ou Marcellus SC.
- **Texte courant / boutons** : Inter, Roboto, Segoe UI, Arial.
- **Tailles UI actuelles** : titre application 34 px, pages 26 px, sections 18 px, texte 13 px, boutons 14 px.
...
> Le `AssetManager` utilise Pillow (`PIL.Image` / `PIL.ImageTk`) pour charger et redimensionner les images. Sur Debian/Kali/Ubuntu, installez `python3-pil` et `python3-pil.imagetk`.
